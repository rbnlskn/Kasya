 ...
 * It only sums transactions with descriptions "Loan Payment" or "Lending Payment".
...
export const calculateTotalPaid = (commitmentId: string, transactions: Transaction[]): number => {
  return transactions
    .filter(t => t.commitmentId === commitmentId && (t.description === 'Loan Payment' || t.description === 'Lending Payment'))
...
export const calculatePaymentsMade = (commitmentId: string, transactions: Transaction[]): number => {
    return transactions.filter(t => t.commitmentId === commitmentId && (t.description === 'Loan Payment' || t.description === 'Lending Payment')).length;
};

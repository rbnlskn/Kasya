...
const WalletFormModal: React.FC<WalletFormModalProps> = ({ isOpen, onClose, onSave, onDelete, initialWallet, currencySymbol, isExiting }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialWallet) {
        ...
        setCustomText(textMatch ? textMatch[1] : '#111827');
      } else {
        setName('');
        setType('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Check if it's a hex or tailwind class - for custom picker it's always hex
    // But for templates it might be different. 
    // Actually, we want to force explicit hex for consistency now.
    const finalBg = customBg.startsWith('#') ? `bg-[${customBg}]` : customBg;
  ...
  const handleTemplateSelect = (t: any) => {
    // Helper to extract hex from either bg-[#hex] or just #hex
    const extractHex = (str: string) => {
      const match = str.match(/#([0-9A-Fa-f]{6})/);
      return match ? match[0] : str; // Fallback to str if no hex found (e.g. named color), but we prefer hex
    };

    const bgHex = extractHex(t.bg);
    const textHex = extractHex(t.text);
        ...
        <div className={`bg-surface w-full max-w-md p-6 rounded-3xl shadow-2xl relative z-10 max-h-[90vh] overflow-y-auto pointer-events-auto ${isExiting ? 'animate-out zoom-out-95 duration-200 fill-mode-forwards' : 'animate-in zoom-in-95 duration-200'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex overflow-x-auto space-x-3 pb-2 -mx-6 px-6 no-scrollbar">
                {WALLET_TEMPLATES.map((template, idx) => (
                  <div
                    key={idx}
                    className="flex-shrink-0 cursor-pointer rounded-2xl hover:ring-2 hover:ring-primary active:scale-95 transition-all p-0.5"
                    onClick={() => handleTemplateSelect(template)}
                    style={{ width: scale(132), height: scale(79) }}
                  >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div onClick={() => { setActiveColorTab('BG'); setIsColorPickerOpen(true); }} className="flex items-center space-x-2 p-2.5 bg-slate-100 dark:bg-slate-800 rounded-lg cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700">
              ...
              <div onClick={() => { setActiveColorTab('TEXT'); setIsColorPickerOpen(true); }} className="flex items-center space-x-2 p-2.5 bg-slate-100 dark:bg-slate-800 rounded-lg cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700">

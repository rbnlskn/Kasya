...
const TransactionFormModal: React.FC<TransactionFormModalProps> = ({ isOpen, onClose, categories, wallets, onSave, onDelete, initialTransaction, currencySymbol, title, isExiting }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialTransaction) {
        ...
        setShowNote(!!initialTransaction.note);
        setIsOffset(initialTransaction.exclude_from_cashflow || false);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      walletId: isOffset ? 'OFFSET' : selectedWallet, // Use dummy ID for offset
        ...
        <div className={`bg-surface w-full max-w-md p-6 rounded-3xl shadow-2xl relative z-10 max-h-[90vh] overflow-y-auto pointer-events-auto ${isExiting ? 'animate-out zoom-out-95 duration-200 fill-mode-forwards' : 'animate-in zoom-in-95 duration-200'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="flex bg-slate-100 p-1 rounded-2xl overflow-x-auto no-scrollbar">
              ...
              <button type="button" onClick={() => setType(TransactionType.TRANSFER)} className={`flex-1 py-2.5 px-2 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${type === TransactionType.TRANSFER ? 'bg-surface shadow-sm text-blue-500 scale-[1.02]' : 'text-text-secondary hover:text-text-primary'}`}>Transfer</button>
              <button type="button" onClick={() => setType(TransactionType.REFUND)} className={`flex-1 py-2.5 px-2 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${type === TransactionType.REFUND ? 'bg-surface shadow-sm text-purple-600 scale-[1.02]' : 'text-text-secondary hover:text-text-primary'}`}>Refund</button>
            </div>
                  ...
                  {showNote && (
                    <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-100 border-2 border-transparent focus:border-primary focus:bg-surface rounded-lg px-4 text-base font-medium text-text-primary outline-none transition-all placeholder-slate-400 h-11"
                        placeholder={
                          type === TransactionType.REFUND ? "e.g. Returned defective item" :
                            type === TransactionType.EXPENSE ? "e.g. Lunch with team" :
                              type === TransactionType.INCOME ? "e.g. Freelance payment" :
                                type === TransactionType.TRANSFER ? "e.g. Savings allocation" :
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="text-left">
                            ...
                            <div className="text-xs text-text-secondary font-medium">
                              {currencySymbol}
                              {isCreditCard(w)
                                ? getAvailableLimit(w).toLocaleString()

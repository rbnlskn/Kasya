    ...
    <div className="fixed inset-0 z-[70] flex items-end justify-center pointer-events-none pb-safe">
      ...
      <div className={`bg-white w-[95%] max-w-md p-6 rounded-3xl shadow-2xl relative z-10 max-h-[90vh] overflow-y-auto pointer-events-auto mx-auto mb-4 ${isExiting ? 'animate-out slide-out-to-bottom duration-300 fill-mode-forwards' : 'animate-in slide-in-from-bottom duration-300'}`}>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">{initialBill ? 'Edit Bill' : 'New Bill'}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full text-lg font-bold border-b-2 border-gray-200 focus:border-amber-500 outline-none py-1 bg-transparent" placeholder="Electricity, Netflix..." required autoFocus={false} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)} className="w-full pl-6 py-2 text-2xl font-black border-b-2 border-gray-200 focus:border-amber-500 outline-none bg-transparent" placeholder="0.00" required inputMode="decimal" step="0.01" />
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Due Day</label>
